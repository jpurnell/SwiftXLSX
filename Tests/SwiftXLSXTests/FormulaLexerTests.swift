...
final class FormulaLexerTests: XCTestCase {
    ...
    func testLongIdentifierIsNotACellReference() throws {
        ...
        XCTAssertFalse(tokens.contains { if case .cellRef = $0 { return true } else { return false } },
                       "a twenty-letter identifier is not a cell reference")
    }
    ...
    func testVeryLongLetterRunsDoNotOverflow() throws {
        for count in [13, 14, 20, 60] {
            ...
            XCTAssertNoThrow(try FormulaLexer.tokenize(word), "\(count) letters")
        }
    }
    ...
    func testGenuineReferencesStillParse() throws {
        for reference in ["A1", "Z99", "AA1", "XFD1048576", "$B$7"] {
            ...
            XCTAssertTrue(tokens.contains { if case .cellRef = $0 { return true } else { return false } },
                          "\(reference) should lex as a reference")
        }
    }
