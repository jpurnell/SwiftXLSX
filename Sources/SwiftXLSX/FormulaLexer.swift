...
public enum FormulaLexer {
    ...
    private static func parseCellReference(raw: [Character]) -> CellRef? {
        ...
        //
        // Both guards inside the loop are load-bearing, and neither was here before.
        //
        // The bound has to be checked *during* the accumulation, not after it. A word of
        // fourteen or more letters overflows `Int` in base 26, and the check that used to
        // sit below this loop never ran — the process died first. `AnnualIncreasePct2022`
        // is enough, which is to say an ordinary defined name in an ordinary workbook.
        //
        // And a letter has to be an ASCII one. `Character.isLetter` is true for `é`, whose
        // uppercase scalar is 201, so `201 - 64` read it as column 137: a wrong answer
        // rather than a crash, and the harder of the two to notice.
        var col = 0
        for letter in colLetters {
            guard letter.isASCII, let ascii = letter.uppercased().first?.asciiValue else {
                return nil
            }
            let val = Int(ascii) - 64
            guard (1...26).contains(val) else { return nil }
            // Rearranged from `col * 26 + val <= maxColumn` so the multiplication never
            // happens unless its result is known to fit.
            guard col <= (maxColumn - val) / 26 else { return nil }
            col = col * 26 + val
        }
