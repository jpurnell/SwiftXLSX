...
final class WorksheetParser: NSObject, XMLParserDelegate {
    ...
    static func parse(data: Data, into sheet: Worksheet,
                      sharedStrings: [String], styles: ParsedStyleSheet,
                      phonetics: [String?] = []) throws {
        let handler = WorksheetParser(sheet: sheet, sharedStrings: sharedStrings,
                                      styles: styles, phonetics: phonetics)
        ...
        case "s":
            ...
            if let index = Int(currentValue) {
                if index >= 0, index < sharedStrings.count {
                    ...
                    // The reading travels with the cell rather than the string, because a
                    // caller asks `PHONETIC(B2)` about a position, not about a value.
                    if index < phonetics.count, let reading = phonetics[index] {
                        sheet?.setPhonetic(currentCellRef, reading)
                    }
                } else {
