...
public final class Worksheet: @unchecked Sendable {
    ...
    /// Phonetic readings by cell reference, for the cells whose file carried one.
    ///
    /// Sparse deliberately: a workbook with no furigana stores nothing, which is every
    /// workbook outside a Japanese locale. Kept beside the cells rather than inside
    /// ``CellValue`` because a reading annotates a value without changing it.
    private(set) var phonetics: [String: String] = [:]

    /// Records the reading of a cell, as read from a file.
    ///
    /// - Parameters:
    ///   - ref: The cell reference.
    ///   - phonetic: The reading.
    func setPhonetic(_ ref: String, _ phonetic: String) {
        phonetics[ref] = phonetic
    }
