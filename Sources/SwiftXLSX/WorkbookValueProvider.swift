...
public struct WorkbookValueProvider: CellValueProvider, @unchecked Sendable {
    ...
    /// Returns the phonetic reading recorded for a cell, if the file carried one.
    ///
    /// Sparse: a workbook outside a Japanese locale records none, and this answers `nil`
    /// for every cell in it. The reading is stored against the cell rather than against
    /// the shared string, because a caller asks `PHONETIC(B2)` about a position — two
    /// cells may share a value and, in principle, differ in how it is read.
    ///
    /// - Parameter ref: The cell reference to look up.
    /// - Returns: The reading, or `nil` when the cell has none.
    public func phonetic(at ref: CellRef) -> String? {
        guard let sheet = workbook.sheets.first(where: { $0.name == currentSheet }) else {
            return nil
        }
        return sheet.phonetics[ref.reference]
    }
