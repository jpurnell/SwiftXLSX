...
- **A long identifier crashed the lexer**, and with it any process reading the workbook.

  `parseCellReference` accumulated the column in base 26 and checked the bound only
  after the loop, so fourteen or more letters overflowed `Int` before anything could
  reject them: `Swift runtime failure: arithmetic overflow`. The guard below the loop
  never ran.

  This is not an exotic input — `AnnualIncreasePct2022` is enough, which is to say an
  ordinary defined name. It was found by a real workbook while enumerating the corpus for
  something else, and it means `Workbook(xlsxData:)` could be killed by a file rather
  than throwing. The bound is now checked during accumulation, rearranged so the
  multiplication never happens unless its result is known to fit.

- **`é1` was read as a cell reference.** `Character.isLetter` is true for accented
  letters, and `é`'s uppercase scalar is 201, so `201 - 64` gave column 137. A wrong
  answer rather than a crash, and the harder of the two to notice. Column letters must
  now be ASCII.
...
- **Phonetic readings are kept, and reachable by cell reference.**

  0.23.2 stopped `<rPh>` text corrupting cell values by discarding it. This keeps it
  instead: `SharedStringsParser.parseEntries(data:)` returns each value paired with its
  reading, the reader threads them to `WorksheetParser`, and the reading is recorded
  against the *cell* rather than the shared string — because `PHONETIC(B2)` asks about a
  position, and two cells may share a value while differing in how it is read.

  `WorkbookValueProvider.phonetic(at:)` surfaces it, satisfying the requirement added in
  SwiftExcelCore 0.8.0. The map is sparse: a workbook outside a Japanese locale records
  nothing and every lookup answers `nil`.

  `SharedStringsParser.parse(data:)` still returns values only and is unchanged for its
  callers. Requires SwiftExcelCore 0.8.0.

## [0.23.2] - 2026-09-10

### Fixed

- **Furigana was being concatenated into cell values.**

  A Japanese workbook stores the reading of a name in an `<rPh>` sibling of the text
  runs — the cell says 山田, and `<rPh>` says ヤマダ, which is how to pronounce it rather
  than part of what it says. `<rPh>` carries its own `<t>`, and `SharedStringsParser`
  matched `case "t" where inSI`, so it accumulated those too. A two-name string with
  two readings returned 山田ヤマダ太郎タロウ where the value is 山田太郎.

  This was a corrupted value rather than a missing feature: wrong for every function
  that reads the cell, not only for a phonetic one, and invisible to a reader who
  cannot tell the two scripts apart — which is why it survived until someone went
  looking for something else. Ordinary rich text still concatenates its runs, and a
  regression test covers that path alongside.

  The phonetic text is still discarded. Exposing it is a separate change.

## [0.23.1] - 2026-09-08

### Changed

- **`SwiftExcelCore` is pinned `from:` rather than `exact:`.**

  Every recorded justification for the exact pin was a *lower* bound — "from 0.12.0
  it depends on SwiftExcelCore" — and none required exactness. The stricter form had
  a cost: when `DependencyGraph` moved to Core and this package took 0.6.0, any
  consumer still on a SwiftExcelFunctions that wanted Core 0.5.0 could not resolve at
  all. Two `exact:` pins on the same shared package deadlock the moment they differ.

  The underlying constraint is real and unchanged: SwiftPM must unify the family on
  one SwiftExcelCore, because two versions would mean two `CellValue` types and
  nothing would typecheck. `from:` enforces that *better* than `exact:` — it picks
  the highest version satisfying everyone rather than refusing.
...
- **`DependencyGraph` moved to SwiftExcelCore**, where it is built from
  `CellAddress` and `FormulaAST` rather than from a file format.

  Its designated initialiser there is `init(cells:provider:)` — an explicit address
  set and a `CellValueProvider`. That is what the old one already read off a
  `Worksheet`: `sheet.name` and `sheet.cells`, a name and a dictionary from
  reference to value. The type was already written against that abstraction and had
  simply never been given it.

  **Nothing here changes for callers.** `init(workbook:)`,
  `init(sheet:including:)` and `init(workbook:including:)` keep their exact
  signatures and semantics as an extension over the moved type, and
  `import SwiftXLSX` still sees `DependencyGraph` by its plain name through
  `SwiftExcelCoreExports.swift` — the same re-export that already carries
  `CellValue` and `FormulaAST`.

  All 771 tests pass **unmoved**, including the three `DependencyGraph` suites,
  which is the evidence that the move is behaviour-preserving.

  What stays here is the part that genuinely involves a file: enumerating a
  workbook's addresses, which a `CellValueProvider` cannot do — it answers "what is
  at this address?" and cannot be asked "which addresses do you have?". A
  `Worksheet` can, because it holds the dictionary.

- Depends on SwiftExcelCore `0.6.0`.
